<div>
    @can('bale-post.update')
        <button wire:click="toggle" type="button" class="group inline-flex items-center cursor-pointer gap-1.5 py-1 px-2.5 rounded-full text-[11px] font-semibold transition-all focus:outline-none
                    {{ $published
    ...
    @else
        ...
        <span class="inline-flex items-center gap-1.5 py-1 px-2.5 rounded-full text-[11px] font-semibold
            ...
            <span
                class="relative inline-flex rounded-full size-1.5 {{ $published ? 'bg-emerald-500' : 'bg-gray-400 dark:bg-gray-500' }}"></span>
            {{ $published ? __('Published') : __('Draft') }}
        </span>
    @endcan
</div>