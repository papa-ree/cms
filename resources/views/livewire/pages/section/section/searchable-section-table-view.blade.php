                ...
                <table class="w-full">
                    <thead class="bg-gray-50 dark:bg-gray-900/50 border-b border-gray-200 dark:border-gray-700">
                        <tr>
                            ...
                            @endforeach
                            @if($section->content['meta']['enable_upload'] ?? false)
                                <th
                                    class="px-6 py-4 text-left text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wider">
                                    <div class="flex items-center gap-2">
                                        <x-lucide-paperclip class="w-4 h-4 text-purple-600" />
                                        Files
                                    </div>
                                </th>
                            @endif
                            @if($section->content['meta']['enable_social'] ?? false)
                                <th
                                    class="px-6 py-4 text-left text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wider">
                                    <div class="flex items-center gap-2">
                                        <x-lucide-share-2 class="w-4 h-4 text-pink-600" />
                                        Social
                                    </div>
                                </th>
                            @endif
                    ...
                    <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                        @foreach($this->filteredItems as $index => $item)
                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-900/30 transition-colors"
                                ...
                                @if($section->content['meta']['enable_upload'] ?? false)
                                    @php
                                        $attachments = collect($item['uploads'] ?? [])->where('key', 'attachments')->all();
                                    @endphp
                                    <td class="px-6 py-4 text-sm text-gray-700 dark:text-gray-300">
                                        @if(count($attachments) > 0)
                                            <div class="flex flex-wrap gap-1.5">
                                                @foreach($attachments as $upload)
                                                    @if(($upload['file_type'] ?? '') === 'image' || in_array(strtolower(pathinfo($upload['url'] ?? $upload['original_name'], PATHINFO_EXTENSION)), ['jpg','jpeg','png','gif','svg','webp']))
                                                        <a href="{{ $upload['url'] }}" target="_blank" class="block w-8 h-8 rounded border border-gray-200 dark:border-gray-700 overflow-hidden hover:opacity-80 transition-opacity">
                                                            <img src="{{ $upload['url'] }}" class="w-full h-full object-cover" alt="Image">
                                                        </a>
                                                    @else
                                ...
                                @if($section->content['meta']['enable_social'] ?? false)
                                    <td class="px-6 py-4 text-sm text-gray-700 dark:text-gray-300">
                                        <div class="flex flex-wrap gap-2">
                                            @foreach(($section->content['meta']['social_platforms'] ?? []) as $platform)
                                                @php $vals = $item['sm_' . $platform] ?? []; @endphp
                                                @if(count($vals) > 0)
                                                    <div class="flex items-center gap-1 group/sm" title="{{ ucfirst($platform) }}">
                                                        @foreach($vals as $val)
                                                            <a href="{{ $val }}" target="_blank" class="p-1 rounded bg-gray-100 dark:bg-gray-800 hover:bg-pink-100 dark:hover:bg-pink-900/30 text-gray-500 hover:text-pink-600 transition-colors">
                                                                @switch($platform)
                                                                    @case('facebook') <x-lucide-facebook class="w-3.5 h-3.5" /> @break
                                                                    @case('instagram') <x-lucide-instagram class="w-3.5 h-3.5" /> @break
                                                                    @case('youtube') <x-lucide-youtube class="w-3.5 h-3.5" /> @break
                                                                    @case('whatsapp') <x-lucide-phone class="w-3.5 h-3.5" /> @break
                                                                    @case('tiktok') <span class="text-[10px] font-bold">TikTok</span> @break
                                                                    @case('twitter') <x-lucide-twitter class="w-3.5 h-3.5" /> @break
                                                                    @case('linkedin') <x-lucide-linkedin class="w-3.5 h-3.5" /> @break
                                                                    @default <x-lucide-share-2 class="w-3.5 h-3.5" />
                                                                @endswitch
                                                            </a>
                                                        @endforeach
                                                    </div>
                    ...
                    <div class="p-4" wire:key="item-mobile-{{ $itemId }}" data-aos="fade-up">
                        ...
                        <div class="space-y-2">
                            @foreach($availableKeys as $key)
                                ...
                                <div>
                                    <div class="flex items-center gap-1.5 mb-1">
                                        <x-lucide-share-2 class="w-3 h-3 text-pink-600" />
                                        <span class="text-xs font-medium text-gray-600 dark:text-gray-400">Social Media</span>
                                    </div>
                                    <div class="flex flex-wrap gap-2">
                                        @foreach(($section->content['meta']['social_platforms'] ?? []) as $platform)
                                            @php $vals = $item['sm_' . $platform] ?? []; @endphp
                                            @if(count($vals) > 0)
                                                @foreach($vals as $val)
                                                    <a href="{{ $val }}" target="_blank" class="inline-flex items-center gap-1.5 px-2 py-1 bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 rounded text-xs">
                                                        <span class="lowercase">{{ $platform }}</span>
                                                    </a>
                                                @endforeach
                                            @endif
                                        @endforeach
                            ...
                            @if($section->content['meta']['enable_upload'] ?? false)
                                <div>
                                    ...
                                    @endphp
                                    <div class="flex items-center gap-1.5 mb-1">
                                        <x-lucide-paperclip class="w-3 h-3 text-purple-600" />
