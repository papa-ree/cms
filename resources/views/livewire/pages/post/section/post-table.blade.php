            ...
            @foreach ($this->availablePosts as $post)
                <tr wire:key='post-{{ $post->slug }}'
                    ...
                    <td class="w-full py-4 pl-4 pr-3 text-sm font-medium text-gray-900 max-w-0 sm:w-auto sm:max-w-none">
                        <div class="block py-3 pe-6">
                            <div class="flex items-center gap-x-3">
                                ...
                                <div class="grow">
                                    <a href="{{ route('bale.cms.posts.edit', $post->slug) }}" {{ $navigate ? 'wire:navigate.hover' : '' }}
                    ...
                    <td class="hidden px-3 py-4 text-sm text-gray-500 md:table-cell">
                        <span
                            class="block text-sm text-gray-800 dark:text-gray-200">{{ $post->userAuthor->name ?? __('unknown') }}</span>
                        <span class="block text-xs text-gray-500 dark:text-gray-500">{{ __('created at') }}
                            {{ $post->created_at }}</span>
                    </td>
                    ...
                    <td class="hidden px-3 py-4 text-sm text-gray-500 sm:table-cell">
                        @if ($post->published)
                            ...
                            @else
                                <span class="py-1 px-1.5 inline-flex items-center gap-x-1 text-xs font-medium bg-teal-100/50 text-teal-800/50 rounded-full dark:bg-teal-500/5 dark:text-teal-500/50">
                                    {{ __('Published') }}
                                </span>
                            @endcan
                            <span class="block text-xs text-gray-500">{{ __('published at') }}
                        ...
                        @else
                            ...
                            @else
                                <span class="py-1 px-1.5 inline-flex items-center gap-x-1 text-xs font-medium bg-slate-100/50 text-slate-800/50 rounded-full dark:bg-slate-500/5 dark:text-slate-500/50">
                                    {{ __('Unpublished') }}
                                </span>
                            @endcan
                        @endif
